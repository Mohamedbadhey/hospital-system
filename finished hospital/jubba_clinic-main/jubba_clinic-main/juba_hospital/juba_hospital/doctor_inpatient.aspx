...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <ul class="nav nav-tabs" id="patientTabs" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" id="history-tab" data-bs-toggle="tab" href="#historyTab" role="tab" aria-controls="historyTab" aria-selected="false" onclick="loadPatientHistoryDoctor()">
                                <i class="fas fa-clipboard-list"></i> History Taking
                            </a>
                        </li>
                    </ul>
                    <div class="tab-content mt-3" id="patientTabsContent">
                        ...
                        <div id="historyTab" class="tab-pane fade" role="tabpanel" aria-labelledby="history-tab">
                            <div class="row">
                                <!-- Left Column: History Input -->
                                <div class="col-md-6">
                                    <h6 class="text-primary mb-3"><i class="fas fa-edit me-2"></i>Record Patient History</h6>
                                    <div class="mb-3">
                                        <label for="historyTextDoctor" class="form-label fw-bold">Patient History</label>
                                        <textarea class="form-control" id="historyTextDoctor" rows="15" 
                                            placeholder="Enter patient history, symptoms, past medical history, family history, etc..."></textarea>
                                        <small id="historyTextErrorDoctor" class="text-danger"></small>
                                    </div>
                                    
                                    <div class="d-grid gap-2">
                                        <button type="button" class="btn btn-primary btn-lg" onclick="submitPatientHistoryDoctor(); return false;">
                                            <i class="fas fa-save me-2"></i>Save History
                                        </button>
                                        <button type="button" class="btn btn-secondary" onclick="clearHistoryFormDoctor(); return false;">
                                ...
                                <div class="col-md-6">
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        <h6 class="text-primary mb-0"><i class="fas fa-history me-2"></i>History Records</h6>
                                        <button class="btn btn-sm btn-outline-primary" onclick="loadPatientHistoryDoctor(); return false;">
                                            <i class="fas fa-sync me-1"></i>Refresh
                                        </button>
                                    </div>

                                    <!-- History records container -->
                                    <div id="historyRecordsContainerDoctor" style="max-height: 600px; overflow-y: auto;">
                                        <div class="alert alert-info text-center">
                                            <i class="fas fa-info-circle"></i> No history records found. Add a new history record using the form.
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        // ==================== Patient History Functions for Doctor ====================
        
        // Global variable to store editing history ID
        var editingHistoryIdDoctor = null;

        // Load patient history records
        function loadPatientHistoryDoctor() {
            if (!currentPatient) {
            ...
            var prescid = currentPatient.prescid || '';
            ...
            $.ajax({
                ...
                dataType: "json",
                success: function (response) {
                    displayPatientHistoryDoctor(response.d);
                },
                error: function (xhr, status, error) {
                    console.error('Error loading history:', error);
                    $('#historyRecordsContainerDoctor').html(
                        '<div class="alert alert-danger"><i class="fas fa-exclamation-triangle"></i> Error loading history records</div>'
                    );
                }
            });
        }
        ...
        function displayPatientHistoryDoctor(histories) {
            var container = $('#historyRecordsContainerDoctor');
            container.empty();

            if (!histories || histories.length === 0) {
                container.html(
                    '<div class="alert alert-info text-center"><i class="fas fa-info-circle"></i> No history records found. Add a new history record using the form.</div>'
                );
                return;
            }
            ...
            histories.forEach(function (history) {
                ...
                // Edit button
                var editBtn = $('<button class="btn btn-outline-primary btn-sm" type="button"><i class="fas fa-edit"></i> Edit</button>');
                editBtn.on('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    editHistoryDoctor(history.HistoryId, history.HistoryText);
                    return false;
                });
                
                // Delete button
                var deleteBtn = $('<button class="btn btn-outline-danger btn-sm" type="button"><i class="fas fa-trash"></i> Delete</button>');
                deleteBtn.on('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    deleteHistoryDoctor(history.HistoryId);
                    return false;
                });
        ...
        function submitPatientHistoryDoctor() {
            var historyText = $('#historyTextDoctor').val().trim();
            ...
            // Validation
            if (!historyText) {
                $('#historyTextErrorDoctor').text('Please enter patient history');
                return;
            }

            $('#historyTextErrorDoctor').text('');

            // Check if we're editing or creating new
            if (editingHistoryIdDoctor) {
                updateHistoryDoctor(editingHistoryIdDoctor, historyText);
            } else {
                saveNewHistoryDoctor(patientId, prescid, historyText);
            }
        }

        // Save new history record
        function saveNewHistoryDoctor(patientId, prescid, historyText) {
            $.ajax({
                type: "POST",
                ...
                success: function (response) {
                    if (response.d === 'success') {
                        ...
                        $('#historyTextDoctor').val('');
                        loadPatientHistoryDoctor();
                    } else {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
        ...
        function editHistoryDoctor(historyId, historyText) {
            editingHistoryIdDoctor = historyId;
            $('#historyTextDoctor').val(historyText);
            $('#historyTextDoctor').focus();
            
            // Change button text to indicate editing mode
            $('button[onclick="submitPatientHistoryDoctor(); return false;"]').html('<i class="fas fa-save me-2"></i>Update History');
        }

        // Update existing history
        function updateHistoryDoctor(historyId, historyText) {
            $.ajax({
                type: "POST",
        ...
        function deleteHistoryDoctor(historyId) {
            Swal.fire({
                title: 'Are you sure?',
        ...
        // Clear history form
        function clearHistoryFormDoctor() {
            $('#historyTextDoctor').val('');
            $('#historyTextErrorDoctor').text('');
            editingHistoryIdDoctor = null;
            $('button[onclick="submitPatientHistoryDoctor(); return false;"]').html('<i class="fas fa-save me-2"></i>Save History');
        }

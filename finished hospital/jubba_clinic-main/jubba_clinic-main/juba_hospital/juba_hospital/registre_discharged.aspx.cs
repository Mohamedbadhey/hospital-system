...
namespace juba_hospital
{
    public partial class registre_discharged : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static string AddRegistrationCharge(string patientid, string prescid, string chargeConfigId)
        {
            string cs = ConfigurationManager.ConnectionStrings["DBCS"].ConnectionString;
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(cs))
                {
                    ...
                    // Get charge details from charges_config
                    ...
                    {
                        chargeCmd.Parameters.AddWithValue("@chargeId", chargeConfigId);
                        using (SqlDataReader dr = chargeCmd.ExecuteReader())
                        {
                            ...
                            {
                                return "error: Charge configuration not found";
                            }
                        }
                    }
                    ...
                    if (chargeAmount > 0)
                    {
                        string invoiceNumber = "REG-" + DateTimeHelper.Now.ToString("yyyyMMdd") + "-" + patientid.PadLeft(4, '0');
                        ...
                        string chargeInsertQuery = @"INSERT INTO patient_charges 
                            ...
                            VALUES (@patientid, @prescid, 'Registration', @charge_name, @amount, 0, @invoice_number)";

                        using (SqlCommand chargeInsertCmd = new SqlCommand(chargeInsertQuery, con))
                        {
                            chargeInsertCmd.Parameters.AddWithValue("@patientid", patientid);
                            chargeInsertCmd.Parameters.AddWithValue("@prescid", string.IsNullOrEmpty(prescid) ? (object)DBNull.Value : prescid);
                            ...
                            int rowsAffected = chargeInsertCmd.ExecuteNonQuery();
                            ...
                            {
                                return "error: Failed to insert charge";
                            }
                        }
                    }
                    else
        ...
        [WebMethod]
        public static string RevertToOutpatient(string patientid, string prescid)
        {
            string cs = ConfigurationManager.ConnectionStrings["DBCS"].ConnectionString;
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(cs))
                {
                    ...
                    // Update patient table to set patient_type to 'outpatient' and patient_status to 0
                    ...
                    using (SqlCommand cmd = new SqlCommand(updatePatientQuery, con))
                    {
                        cmd.Parameters.AddWithValue("@patientid", patientid);
                        cmd.ExecuteNonQuery();
                    }

                    // Update prescribtion table transaction status if prescid is provided
                    if (!string.IsNullOrEmpty(prescid) && prescid != "0")
                    {
                        string updatePrescQuery = @"
                            UPDATE prescribtion 
                            SET transaction_status = 'pending' 
                            WHERE prescid = @prescid";

                        using (SqlCommand cmd = new SqlCommand(updatePrescQuery, con))
                        {
                            cmd.Parameters.AddWithValue("@prescid", prescid);
                            cmd.ExecuteNonQuery();
                        }

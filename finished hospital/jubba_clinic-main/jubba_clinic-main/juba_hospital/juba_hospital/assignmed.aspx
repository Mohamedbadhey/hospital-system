<%@ Page Title="" Language="C#" MasterPageFile="~/doctor.Master" AutoEventWireup="true" CodeBehind="assignmed.aspx.cs"
    ...
    <asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
        ...
        <style>
            ...
            /* DataTable search box enhancements - Make it prominent */
            .dataTables_filter {
                margin-bottom: 15px;
            ...
            .dataTables_filter input:focus {
                ...
                box-shadow: 0 0 0 0.25rem rgba(102, 126, 234, 0.3) !important;
                outline: none;
            ...
            /* Ensure table doesn't break on tablets */
    ...
    <asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body p-0">
                        ...
                        <ul class="nav nav-tabs nav-fill border-bottom-0" id="patientCareTabs" role="tablist" style="background: linear-gradient(to right, #f8f9fa, #e9ecef);">
                            ...
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" id="history-tab" data-bs-toggle="tab" data-bs-target="#history-pane" type="button" role="tab" onclick="loadPatientHistory()">
                                    <i class="fa fa-clipboard-list fa-lg"></i><br>
                                    <span class="fw-bold">History Taking</span>
                                </button>
                            </li>
                        </ul>
                            ...
                            <div class="tab-pane fade" id="history-pane" role="tabpanel">
                                <div class="card border-0 shadow-sm">
                                    <div class="card-header bg-gradient text-white" style="background: linear-gradient(135deg, #fa709a 0%, #fee140 100%);">
                                        <h5 class="mb-0"><i class="fa fa-clipboard-list me-2"></i>Patient History Taking</h5>
                                    </div>
                                    <div class="card-body">
                                        <div class="row">
                                            <!-- Left Column: History Input -->
                                            <div class="col-md-6">
                                                <h6 class="text-muted mb-3"><i class="fa fa-edit me-2"></i>Record Patient History</h6>
                                                <div class="mb-3">
                                                    <label for="historyText" class="form-label fw-bold">Patient History</label>
                                                    <textarea class="form-control" id="historyText" rows="15" 
                                                        placeholder="Enter patient history, symptoms, past medical history, family history, etc..."></textarea>
                                                    <small id="historyTextError" class="text-danger"></small>
                                                </div>
                                                
                                                <div class="d-grid gap-2">
                                                    <button type="button" class="btn btn-primary btn-lg" onclick="submitPatientHistory(); return false;">
                                                        <i class="fa fa-save me-2"></i>Save History
                                                    </button>
                                                    <button type="button" class="btn btn-secondary" onclick="clearHistoryForm(); return false;">
                                            ...
                                            <div class="col-md-6">
                                                <div class="d-flex justify-content-between align-items-center mb-3">
                                                    <h6 class="text-muted mb-0"><i class="fa fa-history me-2"></i>History Records</h6>
                                                    <button class="btn btn-sm btn-outline-primary" onclick="loadPatientHistory()">
                                                        <i class="fa fa-refresh me-1"></i>Refresh
                                                    </button>
                                                </div>

                                                <!-- History records container -->
                                                <div id="historyRecordsContainer" style="max-height: 600px; overflow-y: auto;">
                                                    <div class="alert alert-info text-center">
                                                        <i class="fa fa-info-circle"></i> No history records found. Add a new history record using the form.
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
        ...
        <%-- <!-- Modal -->
            ...
            <script>
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                if (response.d === 'true') {
                                    ...
                                    // Reload medications table after adding new one
                                    var prescid = $("#id11").val();
                                    if (prescid) {
                                        loadMedications(prescid);
                                    }
                ...
                function initDataTable(attempt = 1) {
                    if (typeof $.fn.DataTable !== 'undefined') {
                        ...
                        window.patientTable = $('#datatable').DataTable({
                            ...
                            columnDefs: [
                                ...
                                { responsivePriority: 2, targets: -1 }, // Action buttons
                                { responsivePriority: 3, targets: 1 } // Patient ID
                            ],
                        ...
                        // Retry up to 40 times (20 seconds) - increased for slower connections
                        if (attempt < 40) {
                            setTimeout(function() {
                                initDataTable(attempt + 1);
                            }, 500);
                        }
                        // Silent fail after 40 attempts - table will just show as plain HTML
                    }
                }

                // Using DataTable's built-in search functionality (no custom code needed)
                // The search box is automatically generated by DataTable and styled via CSS
            ...
            // Removed duplicate - search connection is now handled at line 6296

            // ==================== Patient History Functions ====================
            
            // Global variable to store editing history ID
            var editingHistoryId = null;

            // Load patient history records
            function loadPatientHistory() {
                ...
                $.ajax({
                    type: "POST",
                    url: "assignmed.aspx/GetPatientHistory",
                    data: JSON.stringify({
                        patientId: patientId,
                        prescid: prescid
                    }),
                    ...
                    dataType: "json",
                    success: function (response) {
                        displayPatientHistory(response.d);
                    },
                    error: function (xhr, status, error) {
                        console.error('Error loading history:', error);
                        $('#historyRecordsContainer').html(
                            '<div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> Error loading history records</div>'
                        );
                    }
                });
            }
            ...
            function displayPatientHistory(histories) {
                var container = $('#historyRecordsContainer');
                container.empty();

                if (!histories || histories.length === 0) {
                    container.html(
                        '<div class="alert alert-info text-center"><i class="fa fa-info-circle"></i> No history records found. Add a new history record using the form.</div>'
                    );
                    return;
                }

                histories.forEach(function (history) {
                    var card = $('<div class="card mb-3 shadow-sm"></div>');
                    var cardBody = $('<div class="card-body"></div>');
                    
                    // Header with date and buttons
                    var header = $('<div class="d-flex justify-content-between align-items-start mb-2"></div>');
                    var dateInfo = $('<div><small class="text-muted"><i class="fa fa-calendar"></i> ' + history.CreatedDate + 
                        (history.LastUpdated ? ' (Updated: ' + history.LastUpdated + ')' : '') + '</small></div>');
                    
                    var btnGroup = $('<div class="btn-group btn-group-sm"></div>');
                    
                    // Edit button
                    var editBtn = $('<button class="btn btn-outline-primary btn-sm" type="button"><i class="fa fa-edit"></i> Edit</button>');
                    editBtn.on('click', function(e) {
                        e.preventDefault();
                        e.stopPropagation();
                        editHistory(history.HistoryId, history.HistoryText);
                        return false;
                    });
                    
                    // Delete button
                    var deleteBtn = $('<button class="btn btn-outline-danger btn-sm" type="button"><i class="fa fa-trash"></i> Delete</button>');
                    ...
                    var historyContent = $('<div class="history-text" style="white-space: pre-wrap; background: #f8f9fa; padding: 15px; border-radius: 5px; border-left: 4px solid #667eea;"></div>');
            ...
            // Submit or update patient history
            function submitPatientHistory() {
                var historyText = $('#historyText').val().trim();
                var patientId = $('#pid').val();
                var prescid = $('#id11').val();

                // Validation
                if (!historyText) {
                    $('#historyTextError').text('Please enter patient history');
                    return;
                }

                $('#historyTextError').text('');

                // Check if we're editing or creating new
                if (editingHistoryId) {
                    updateHistory(editingHistoryId, historyText);
                } else {
                    saveNewHistory(patientId, prescid, historyText);
                }
            }

            // Save new history record
            function saveNewHistory(patientId, prescid, historyText) {
                $.ajax({
                    type: "POST",
                    url: "assignmed.aspx/SavePatientHistory",
                    data: JSON.stringify({
                        patientId: patientId,
                        prescid: prescid,
                    ...
                    success: function (response) {
                        if (response.d === 'success') {
                            Swal.fire({
                                icon: 'success',
                                title: 'Success!',
                                text: 'History record saved successfully!',
                                timer: 2000,
                                showConfirmButton: false
                            });
                            $('#historyText').val('');
                    ...
                    error: function (xhr, status, error) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Failed',
                            text: 'Failed to save history record',
                            confirmButtonColor: '#d33'
                        });
                    }
                });
            ...
            // Edit history - populate form
            function editHistory(historyId, historyText) {
                editingHistoryId = historyId;
                $('#historyText').val(historyText);
                $('#historyText').focus();
                
                // Change button text to indicate editing mode
                $('button[onclick="submitPatientHistory()"]').html('<i class="fa fa-save me-2"></i>Update History');
            }

            // Update existing history
            function updateHistory(historyId, historyText) {
                $.ajax({
                    type: "POST",
                    url: "assignmed.aspx/UpdatePatientHistory",
                    data: JSON.stringify({
                        historyId: historyId,
                        historyText: historyText,
                    ...
                    success: function (response) {
                        if (response.d === 'success') {
                            Swal.fire({
                                icon: 'success',
                                title: 'Updated!',
                                text: 'History record updated successfully!',
                                timer: 2000,
                                showConfirmButton: false
                            });
                            clearHistoryForm();
                    ...
                    error: function (xhr, status, error) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Failed',
                            text: 'Failed to update history record',
                            confirmButtonColor: '#d33'
                        });
                    }
                });
            ...
            function deleteHistory(historyId) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "POST",
                            url: "assignmed.aspx/DeletePatientHistory",
                            data: JSON.stringify({
                                historyId: historyId
                            }),
                            contentType: "application/json; charset=utf-8",
            ...
            function clearHistoryForm() {
                ...
                $('button[onclick="submitPatientHistory()"]').html('<i class="fa fa-save me-2"></i>Save History');
            }

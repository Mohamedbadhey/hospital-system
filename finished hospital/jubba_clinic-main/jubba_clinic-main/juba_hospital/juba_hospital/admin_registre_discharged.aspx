<%@ Page Title="Discharged Patients" Language="C#" MasterPageFile="~/Admin.Master" AutoEventWireup="true" CodeBehind="registre_discharged.aspx.cs" Inherits="juba_hospital.registre_discharged" %>
    ...
    <script>
        ...
        $('#filterType').on('change', function () {
            ...
            if (value === '') {
                ...
                $('.patient-card').each(function () {
                    var patientType = $(this).data('patient-type');
                    $(this).toggle(patientType === value);
                });
            }
        });
        ...
        $('#filterPayment').on('change', function () {
            ...
            if (value === '') {
                ...
                $('.patient-card').each(function () {
                    var unpaid = parseFloat($(this).find('.unpaid-status').text().replace('Unpaid: $', ''));
                    $(this).toggle(unpaid === 0);
                });
        ...
        function applyDateFilter() {
            var fromDate = new Date($('#filterDateFrom').val());
            var toDate = new Date($('#filterDateTo').val());
            ...
            $('.patient-card').each(function () {
                var dateText = $(this).find('p:contains("Discharged:")').text();
                var match = dateText.match(/Discharged: (.+)/);
                if (match) {
                    var cardDate = new Date(match[1]);
                    var show = true;
                    
                    if (!isNaN(fromDate) && cardDate < fromDate) show = false;
                    if (!isNaN(toDate) && cardDate > toDate) show = false;
        ...
        $('#filterDateFrom, #filterDateTo').on('change', applyDateFilter);
    </script>
</asp:Content>

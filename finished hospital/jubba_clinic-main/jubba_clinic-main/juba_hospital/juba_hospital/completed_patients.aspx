...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" id="desktopSearchInput" placeholder="Search by name, phone, location..." onkeyup="performDesktopSearch()">
                                    <button class="btn btn-outline-secondary" type="button" onclick="clearDesktopSearch()">
    ...
    <script type="text/javascript">
        ...
        function viewDetails(prescid) {
            ...
            var patient = allCompletedPatientsData.find(p => p.prescid == prescid);
            if (patient) {
                window.open('completed_patient_full_report.aspx?patientid=' + patient.patientid + '&prescid=' + prescid, '_blank');
            } else {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
        ...
        function markAsPending(prescid) {
            ...
            Swal.fire({
                ...
                text: 'Do you want to add a registration charge for this patient?',
                icon: 'question',
                ...
                denyButtonText: 'No, Just Mark Pending',
                cancelButtonText: 'Cancel'
            }).then((result) => {
                if (result.isConfirmed) {
                    // User wants to add registration charge - load charges
                    loadRegistrationChargesForPending(prescid);
                } else if (result.isDenied) {
                    // User just wants to mark as pending without charge
                    updateTransactionStatus(prescid, 'pending', null);
                }
            });
        }

        function loadRegistrationChargesForPending(prescid) {
            // Get patient data
            var patient = allCompletedPatientsData.find(p => p.prescid == prescid);
            if (!patient) {
                Swal.fire({
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.d && response.d.length > 0) {
                        showRegistrationChargeSelector(prescid, patient.patientid, response.d);
                    } else {
                        Swal.fire({
                            icon: 'warning',
                            title: 'No Charges Available',
                            text: 'No registration charges configured. Marking as pending without charge.',
                            timer: 2000
                        }).then(() => {
                            updateTransactionStatus(prescid, 'pending', null);
                        });
                ...
                error: function (error) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Failed to load registration charges. Marking as pending without charge.'
                    }).then(() => {
                        updateTransactionStatus(prescid, 'pending', null);
                    });
                }
        ...
        function showRegistrationChargeSelector(prescid, patientid, charges) {
            // Build options HTML
            let optionsHtml = '<select id="swal-registration-charge" class="swal2-input" style="width: 80%;">';
            ...
            Swal.fire({
                ...
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#6c757d',
                confirmButtonText: 'Apply Charge & Mark Pending',
                cancelButtonText: 'Cancel',
                preConfirm: () => {
                    const chargeId = document.getElementById('swal-registration-charge').value;
                    if (chargeId === '0') {
                ...
                if (result.isConfirmed) {
                    // Add registration charge and mark as pending
                    addRegistrationChargeAndMarkPending(prescid, patientid, result.value);
                }
            });
        }

        function addRegistrationChargeAndMarkPending(prescid, patientid, chargeConfigId) {
            $.ajax({
                type: "POST",
                url: "completed_patients.aspx/AddRegistrationCharge",
                data: JSON.stringify({
                ...
                success: function (response) {
                    if (response.d === "success") {
                        // Now mark as pending
                        updateTransactionStatus(prescid, 'pending', chargeConfigId);
                    } else {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
        ...
        function updateTransactionStatus(prescid, status, chargeConfigId) {
            $.ajax({
                type: "POST",
                ...
                success: function (response) {
                    if (response.d === "success") {
                        let successMessage = chargeConfigId ? 
                            'Patient marked as ' + status + ' with registration charge applied' : 
                            'Patient marked as ' + status;
        ...
        function performDesktopSearch() {
            var searchTerm = $('#desktopSearchInput').val().toLowerCase();

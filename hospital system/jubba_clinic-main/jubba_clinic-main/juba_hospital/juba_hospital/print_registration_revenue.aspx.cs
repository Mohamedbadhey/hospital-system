...
{
    public partial class print_registration_revenue : System.Web.UI.Page
    {
            ...
            using (SqlCommand cmd = new SqlCommand(query, con))
            {
                using (SqlDataReader dr = cmd.ExecuteReader())
                {
                    if (dr.Read())
                    {
                        ...
                        int totalCount = Convert.ToInt32(dr["total_count"]);
                        decimal avgFee = Convert.ToDecimal(dr["average_fee"]);
                        int pendingCount = Convert.ToInt32(dr["pending_count"]);

                        litTotalRevenue.Text = totalRevenue.ToString("N2");
                        litTotalCount.Text = totalCount.ToString();
                        litAvgFee.Text = avgFee.ToString("N2");
        ...
        private void LoadCharges(SqlConnection con, string dateCondition, string paymentCondition, string searchCondition)
        {
            ...
            using (SqlCommand cmd = new SqlCommand(query, con))
            {
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    ...
                    da.Fill(dt);
                    rptRegistrationCharges.DataSource = dt;
                    rptRegistrationCharges.DataBind();
                }
            }
        }

<%@ Page Title="Discharged Patients" Language="C#" MasterPageFile="~/doctor.Master" AutoEventWireup="true" CodeBehind="registre_discharged.aspx.cs" Inherits="juba_hospital.registre_discharged" %>
    ...
    <script>
        ...
        function printAllDischarged() {
            ...
            if (patientIds.length === 0) {
                Swal.fire('No Patients', 'No patients to print.', 'info');
                return;
            }
            window.open('print_all_discharged.aspx?patientids=' + patientIds.join(','), '_blank');
        }
        ...
        function applyFilters() {
            ...
            $('.patient-card').each(function () {
                ...
                if (searchValue !== '' && $card.text().toLowerCase().indexOf(searchValue) === -1) showCard = false;

                if (showCard && typeFilter !== '' && $card.text().toLowerCase().indexOf(typeFilter) === -1) showCard = false;
                ...
                if (showCard && paymentFilter !== '') {
                    ...
                    if (paymentFilter === 'paid' && unpaidAmount > 0) showCard = false;
                    else if (paymentFilter === 'unpaid' && unpaidAmount === 0) showCard = false;
                }
                ...
                if (showCard && (fromDate || toDate)) {
                    ...
                    if (match) {
                        ...
                        if (fromDate && !isNaN(fromDate) && cardDate < fromDate) showCard = false;
                        if (toDate && !isNaN(toDate) && cardDate > toDate) showCard = false;
                    }
                }
        ...
        function updatePatientCount() {
            ...
            countDisplay.text('Showing ' + visibleCount + ' of ' + totalCount);
        }
        ...
        waitForJQuery(function() {
            $(document).ready(function() {
                updatePatientCount();
                $('#searchPatient').on('input keyup', applyFilters);
                $('#filterType').on('change', applyFilters);
                $('#filterPayment').on('change', applyFilters);

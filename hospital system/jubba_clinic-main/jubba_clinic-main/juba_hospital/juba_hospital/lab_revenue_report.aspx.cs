...
{
    public partial class lab_revenue_report : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static LabReportData GetLabReport(string dateRange, string startDate, string endDate, string paymentStatus, string patientSearch)
        {
            LabReportData report = new LabReportData();
            ...
            using (SqlConnection con = new SqlConnection(cs))
            {
                ...
                report.testDistribution = GetTestDistribution(con, dateCondition, paymentCondition, searchCondition);
            }
        ...
        private static LabStatistics GetStatistics(SqlConnection con, string dateCondition, string paymentCondition, string searchCondition)
        {
            ...
            string query = $@"
                ...
                WHERE pc.charge_type = 'Lab' AND {dateCondition} {paymentCondition} {searchCondition}";
        ...
        private static List<LabDetail> GetDetails(SqlConnection con, string dateCondition, string paymentCondition, string searchCondition)
        {
            List<LabDetail> details = new List<LabDetail>();
            string query = $@"
        ...
        private static List<TestDistribution> GetTestDistribution(SqlConnection con, string dateCondition, string paymentCondition, string searchCondition)
        {
            List<TestDistribution> distribution = new List<TestDistribution>();
            string query = $@"
                SELECT TOP 5 pc.charge_name AS test_name, SUM(CASE WHEN pc.is_paid = 1 THEN pc.amount ELSE 0 END) AS revenue
                FROM patient_charges pc INNER JOIN patient p ON pc.patientid = p.patientid
                WHERE pc.charge_type = 'Lab' AND {dateCondition} {paymentCondition} {searchCondition}
            ...
            using (SqlCommand cmd = new SqlCommand(query, con))
            {
                using (SqlDataReader dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        distribution.Add(new TestDistribution
                        {
        ...
        {
            public LabStatistics statistics { get; set; }
            public List<LabDetail> details { get; set; }
            public List<DailyBreakdown> dailyBreakdown { get; set; }
            public List<TestDistribution> testDistribution { get; set; }
        }
